...
void setup() {
  ...
  #if ENABLED(USE_Z_SENSORLESS)
    use_z_sensorless.init();
  #endif

  #if ENABLED(NO_MOTION_BEFORE_HOMING)
    anker_homing.anker_disable_motton_before_init();
  #endif
...
void loop() {
  do {
    idle();
      ...
      settings.settings_params_monitor();
    #endif
    queue.advance();
